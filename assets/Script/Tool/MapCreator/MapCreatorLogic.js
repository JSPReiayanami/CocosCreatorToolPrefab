...
cc.Class({
  ...
  DelItemList(logicId) {
    let itemList = this.m_ItemLists[logicId]
    if (itemList != null) {
      itemList.Destoy()
      delete this.m_ItemLists[logicId]
    }
  },
  ShowClickItemLogicView(listItem, item) {
    ...
    for (let disId in disListIdArr) {
      this.DelItemList(disListIdArr[disId])
    }
    let logicId = item.GetLogicId()
    listItem.SetItemChoice(logicId)
    if(this.GetItemListWithLogicId(logicId) != null) return;
    let newItemList = this.AddItemList({LogicId: logicId})
    listItem.SetNextId(logicId)
    this.RefreshItemListView()
  },
  RefreshItemListView() {
