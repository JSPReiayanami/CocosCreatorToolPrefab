...
class GIF {
  ...
  private _spriteBase64Frames: Array<String> = [];
  ...
  handle(item, callback) {
    ...
    if (this._spriteFrames.length == 0) {
      ...
      callback(null, {
        ...
        spriteBase64Frames: this._spriteBase64Frames
      })
    }
  }
  ...
  private dataUrl2SpriteFrame(dataUrl) {
    ...
    return new cc.SpriteFrame(textureObj);
  }
  ...
  private decodeFrame2Texture(frame, index) {
    ...
    if (CC_JSB) {
      ...
      this._spriteBase64Frames[index] = dataUrl;
    }
...
class GIFCache {
  ...
  static async parseBase64ToGif(arrayBuffer: ArrayBuffer, onComplete) {
    let gif = new GIF();
    gif.handle(arrayBuffer, onComplete)
  }
